...
test('card choice clearly announces that the payment is coming soon', async ({ page }) => {
  const errors = [];
  ...
  await expect(page.getByText('Paiement par carte bientôt disponible').last()).toBeVisible();
  await expect(page.getByText(/Pour commander maintenant, choisissez le paiement à la livraison/).last()).toBeVisible();
  await expect(page.getByRole('button', { name: 'Paiement par carte bientôt disponible' })).toBeDisabled();
